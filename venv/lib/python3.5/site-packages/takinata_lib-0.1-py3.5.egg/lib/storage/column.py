...
class ColumnStorage:
    ...
    def get_column(cls, project_name, name):
        ...
        :return:
        """
        f_column = ColumnStorage.session.query(Category).filter(Category.name == name)
        project = ProjectStorage.get_project(project_name)
        column = ColumnStorage.session.query(f_column).filter(project_id == project.id)
        ColumnStorage.session.close()
        return column
    ...
    def get_all_columns(cls, project_name):
        ...
        :return:
        """
        columns = ColumnStorage.session.query(Category).all()
        ColumnStorage.session.close()
        return columns
