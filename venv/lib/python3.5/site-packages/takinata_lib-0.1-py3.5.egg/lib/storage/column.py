...
engine = create_engine('sqlite:////home/snitch/PycharmProjects/VersionTwo/venv/lib/python3.5/site-packages/takinata_lib-0.1-py3.5.egg/database.sqlite3', echo=False)
...
class ColumnStorage:
    ...
    def add_column_to_db(cls, column):
        ...
        for i in all_columns:
            if i.name == column.name:
                have = True
        if not have:
            ColumnStorage.session.add(column)
    ...
    def get_all_columns(cls, project_name):
        ...
        """
        cols = []
        project = ColumnStorage.session.query(Project).filter(Project.name == project_name).first()
