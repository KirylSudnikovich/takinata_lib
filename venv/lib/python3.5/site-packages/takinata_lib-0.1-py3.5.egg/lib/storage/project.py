...
class ProjectStorage:
    ...
    @classmethod
    def show_all_for_user(cls, user):
        """
        Displays a list of projects where user with specified id is a creator
        :return: project list
        """
        projects = session.query(Project).filter(Project.user_id == str(user.id)).all()
        session.close()
        return projects
