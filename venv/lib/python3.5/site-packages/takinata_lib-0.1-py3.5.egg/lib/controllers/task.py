...
class TaskController:
    ...
    def add_task(cls, username, password, project_id, category_id, name, desc, type, start_date, start_time, end_date,
        ...
        if user.password == password:
            ...
            if start_date == "" or end_date == "":
                ...
                if end < start:
                    TaskController.log.error("EndDate is before StartDate")
                    raise EndBeforeStart
                if start < today:
                    TaskController.log.error("StartDate is before today")
                    raise StartBeforeToday
                if end < today:
                    TaskController.log.error("EndDate is before today")
            ...
            if not have:
                ...
                TaskController.log.info("Task was successfully created")
                return task
            else:
    ...
    def delete_task(cls, task_id):
        ...
        if task.is_subtask == 1:
            ...
            if can:
                TaskStorage.delete_task_from_db(task)
                TaskController.log.info("Task was successfully deleted")
            else:
    ...
    def cancel_task(cls, task):
        ...
        if task.assosiated_task_id != None:
            ...
            TaskController.log.info("Task was cancel")
        else:
            TaskStorage.cancel_task(task)
            TaskController.log.info("Task was cancel")
        ...
        if task1.is_subtask == 0:
            if task1.start_date > task2.start_date and task1.end_date < task2.end_date:
                if task1.priority > task2.priority:
                    TaskController.log.error("Subtask priority not valid")
                ...
                TaskController.log.error("Subtask date error")
                ...
                if task1.first_date > task2.first_date and task1.second_date < task2.second_date:
                    if task1.priority > task2.priority:
                        ...
                        if can:
                            ...
                            TaskController.log.error("This task is already subtask")
                            raise AlreadySubtask
                else:
                    TaskController.log.error("Subtask date error")
                    raise SubtaskDateException
    ...
    def start_again(cls, task):
        ...
        TaskController.log.info("Dates for regular task were reloaded")
